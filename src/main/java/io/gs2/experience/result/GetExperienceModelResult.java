...
public class GetExperienceModelResult implements IResult, Serializable {
	...
	public GetExperienceModelResult withItem(ExperienceModel item) {
		this.item = item;
		return this;
	}

    public static GetExperienceModelResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new GetExperienceModelResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : ExperienceModel.fromJson(data.get("item")));
    }
