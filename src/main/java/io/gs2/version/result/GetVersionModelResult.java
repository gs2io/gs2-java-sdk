...
public class GetVersionModelResult implements IResult, Serializable {
	...
	public GetVersionModelResult withItem(VersionModel item) {
		this.item = item;
		return this;
	}

    public static GetVersionModelResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new GetVersionModelResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : VersionModel.fromJson(data.get("item")));
    }
