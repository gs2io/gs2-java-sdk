...
public class GetLimitModelResult implements IResult, Serializable {
	...
	public GetLimitModelResult withItem(LimitModel item) {
		this.item = item;
		return this;
	}

    public static GetLimitModelResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new GetLimitModelResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : LimitModel.fromJson(data.get("item")));
    }
