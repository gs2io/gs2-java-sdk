...
public class GetQuestModelResult implements IResult, Serializable {
	...
	public void setItem(QuestModel item) {
		this.item = item;
	}

	public GetQuestModelResult withItem(QuestModel item) {
		this.item = item;
		return this;
	}

    public static GetQuestModelResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new GetQuestModelResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : QuestModel.fromJson(data.get("item")));
    }
