...
public class CurrentVersionMasterDomain {
    ...
    public CurrentVersionMasterDomain exportMaster(
        ...
        if (result.getItem() != null) {
            cache.put(
                parentKey,
                io.gs2.version.domain.model.CurrentVersionMasterDomain.createCacheKey(
                ),
                result.getItem(),
    ...
    private CurrentVersionMaster get(
        GetCurrentVersionMasterRequest request
    ) {
        request
            .withNamespaceName(this.namespaceName);
        GetCurrentVersionMasterResult result = this.client.getCurrentVersionMaster(
            request
        );
    ...
    public CurrentVersionMasterDomain update(
        UpdateCurrentVersionMasterRequest request
    ) {
        request
            .withNamespaceName(this.namespaceName);
        UpdateCurrentVersionMasterResult result = this.client.updateCurrentVersionMaster(
            request
        );
    ...
    public CurrentVersionMasterDomain updateFromGitHub(
        UpdateCurrentVersionMasterFromGitHubRequest request
    ) {
        request
            .withNamespaceName(this.namespaceName);
        UpdateCurrentVersionMasterFromGitHubResult result = this.client.updateCurrentVersionMasterFromGitHub(
            request
        );
    ...
    public CurrentVersionMaster model() {
        CurrentVersionMaster value = cache.get(
            parentKey,
            io.gs2.version.domain.model.CurrentVersionMasterDomain.createCacheKey(
            ),
            CurrentVersionMaster.class
        );
        if (value == null) {
            try {
                this.get(
