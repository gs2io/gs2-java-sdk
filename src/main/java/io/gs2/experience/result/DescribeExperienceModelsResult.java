...
public class DescribeExperienceModelsResult implements IResult, Serializable {
	...
	public DescribeExperienceModelsResult withItems(List<ExperienceModel> items) {
		this.items = items;
		return this;
	}

    public static DescribeExperienceModelsResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new DescribeExperienceModelsResult()
            .withItems(data.get("items") == null || data.get("items").isNull() ? new ArrayList<ExperienceModel>() :
    ...
    public JsonNode toJson() {
        return new ObjectMapper().valueToTree(
            new HashMap<String, Object>() {{
                put("items", getItems() == null ? new ArrayList<ExperienceModel>() :
