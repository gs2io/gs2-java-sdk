...
public class UpdatePrizeTableMasterResult implements IResult, Serializable {
	...
	public UpdatePrizeTableMasterResult withItem(PrizeTableMaster item) {
		this.item = item;
		return this;
	}

    public static UpdatePrizeTableMasterResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new UpdatePrizeTableMasterResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : PrizeTableMaster.fromJson(data.get("item")));
    }
