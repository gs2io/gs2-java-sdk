...
package io.gs2.dictionary.domain;
...
import io.gs2.dictionary.domain.model.*;
...
public class Gs2Dictionary {
    ...
    public NamespaceDomain createNamespace(
        ...
        String parentKey = "dictionary:Namespace";
                
        if (result.getItem() != null) {
            cache.put(
                parentKey,
                io.gs2.dictionary.domain.model.NamespaceDomain.createCacheKey(
        ...
        try {
            switch (method) {
                case "AddEntriesByUserId": {
                    AddEntriesByUserIdRequest requestModel = AddEntriesByUserIdRequest.fromJson(new ObjectMapper().readTree(request));
                    AddEntriesByUserIdResult resultModel = AddEntriesByUserIdResult.fromJson(new ObjectMapper().readTree(result));
                    for (Entry item : resultModel.getItems()) {
                        String parentKey = UserDomain.createCacheParentKey(
                            requestModel.getNamespaceName().toString(),
                            item.getUserId().toString(),
                            "Entry"
                        );
                        String key = EntryDomain.createCacheKey(
        ...
        try {
            switch (method) {
                case "add_entries_by_user_id": {
                    AddEntriesByUserIdRequest requestModel = AddEntriesByUserIdRequest.fromJson(new ObjectMapper().readTree(job.getArgs()));
                    AddEntriesByUserIdResult resultModel = AddEntriesByUserIdResult.fromJson(new ObjectMapper().readTree(result.getResult()));
                    for (Entry item : resultModel.getItems()) {
                        String parentKey = UserDomain.createCacheParentKey(
                            requestModel.getNamespaceName().toString(),
                            item.getUserId().toString(),
