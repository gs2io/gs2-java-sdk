...
public class SalesItemGroupMaster implements IModel, Serializable, Comparable<SalesItemGroupMaster> {
    ...
    public static SalesItemGroupMaster fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new SalesItemGroupMaster()
            .withSalesItemGroupId(data.get("salesItemGroupId") == null || data.get("salesItemGroupId").isNull() ? null : data.get("salesItemGroupId").asText())
    ...
    public JsonNode toJson() {
        return new ObjectMapper().valueToTree(
            new HashMap<String, Object>() {{
                put("salesItemGroupId", getSalesItemGroupId());
                put("name", getName());
