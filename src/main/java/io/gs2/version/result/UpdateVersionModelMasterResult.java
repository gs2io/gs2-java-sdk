...
public class UpdateVersionModelMasterResult implements IResult, Serializable {
	...
	public UpdateVersionModelMasterResult withItem(VersionModelMaster item) {
		this.item = item;
		return this;
	}

    public static UpdateVersionModelMasterResult fromJson(JsonNode data) {
        if (data == null) {
            return null;
        }
        return new UpdateVersionModelMasterResult()
            .withItem(data.get("item") == null || data.get("item").isNull() ? null : VersionModelMaster.fromJson(data.get("item")));
    }
